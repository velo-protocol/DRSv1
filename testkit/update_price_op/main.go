...
func main() {
	veloTxB64 := buildB64UpdatePriceOp(helper.PublicKeyTP, "VELO", "THB", "1", helper.KPTP)

	helper.DecodeB64VeloTx(veloTxB64)
	helper.CompareVeloTxSigner(veloTxB64, helper.PublicKeyRegulator)
}

func buildB64UpdatePriceOp(txSourceAccount, asset, currency, priceInCurrencyPerAssetUnit string, secretKey *keypair.Full) string {
	fmt.Println("##### Start Build Update Price Operation #####")
	...
	fmt.Println("##### End Build Update Price Operation #####")
